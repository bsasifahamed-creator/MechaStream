...
<body>
  <div class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="text-center p-8">
      <h1 class="text-2xl font-bold text-gray-900">Your Project</h1>
      <p class="mt-2 text-gray-600">Generated by AI. Edit your code in the IDE and run again to see changes.</p>
    </div>
  </div>
...
User request: ${prompt}`,
    ...
    let content = (data.response || data?.message?.content || '').trim();
    content = content.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
    if (!content) return null;
    if (process.env.NODE_ENV === 'development') {
      console.log('Generate: Ollama raw response length:', content.length);
      console.log('Generate: Ollama first 500 chars:', content.slice(0, 500));
    }
    ...
    if (err.name === 'AbortError') {
      console.error('Generate: Ollama request timed out after 180 seconds');
    } else {
      console.error('Generate: Ollama error:', err.name, err.message);
    }
    console.log('Generate: Ollama unavailable or timeout, using template:', err.message);
    return null;
  }
}

/** Try Ollama for AI-generated files; fall back to template so Build never hangs. */
async function getProjectNameAndFilesAsync(prompt: string): Promise<{ projectName: string; files: ProjectFile[] }> {
  ...
  const fromOllama = await tryGetFilesFromOllama(OLLAMA_BASE_URL, prompt);
  if (fromOllama && fromOllama.files.length > 0) {
    return { projectName: fromOllama.projectName, files: fromOllama.files };
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    const { projectName, files } = await getProjectNameAndFilesAsync(prompt);
    writeProjectToDisk(projectName, files);
    const ideUrl = new URL('/ide', origin);
    ideUrl.search = `?project=${encodeURIComponent(projectName)}&prompt=${encodeURIComponent(prompt)}&openChat=1`;
    return NextResponse.redirect(ideUrl.toString(), 307);
  } catch (e) {
